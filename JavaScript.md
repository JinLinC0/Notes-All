...
JavaScript BigInt 变量用于存储太大而无法用普通 JavaScript 数字表示的大整数值，普通的JavaScript 整数最多只能精确到 15 位，如需创建 BigInt，可以在整数末尾添加 `n`，或调用 `BigInt()` 函数
...
可用于 JavaScript Number 的运算符也可用于 BigInt，但是不允许在 BigInt 和 Number 之间进行算术运算（类型转换会丢失信息）

BigInt 无法进行无符号右移操作（>>>），因为它没有固定的宽度

BigInt 不能有小数

将BigInt类型的数转化为Number类型的数的方法为：Number(x)
...
|     方法     |                             描述                             |
| :----------: | :----------------------------------------------------------: |
|   Number()   | 返回从其参数转换而来的数字，原始的字符串允许有空格，如果是字符串是小数，则返回该小数，不支持多个转化多个数据，会返回`NaN`，如果无法转换数字，则返回 `NaN` |
...
这些属性只能作为 Number.MAX_VALUE 来访问，使用其他访问将返回`undefined`
...
其次，我们尝试用一个数值来和一个字符串进行运算的时候，得到的结果也是NaN，如`console.log(2 / "jjj")`

同时NaN类型是不能和NaN类型进行比较的，如：`console.log(NaN == NaN)  //结果显示false`
...
可以在字符串中使用引号，只要不匹配围绕字符串的引号即可，外面双里面单，外面单里面双，如果一定要内外使用一样的引号，可以使用转义字符来分隔内部的引号
...
字符串原始值是无法拥有属性和方法（因为它们不是对象），但是通过 JavaScript，方法和属性也可用于原始值，因为在执行方法和属性时 JavaScript 将原始值视为对象

|         方法          |                             描述                             |
| :-------------------: | :----------------------------------------------------------: |
|        length         |     返回字符串的长度，如：txt.length，空格也会算在长度中     |
...
|      endsWith()       | 判断字符串是否以指定值结尾，具体形式为：string.endsWith(searchvalue, length)，第二个参数表示要搜索的长度 |
|   slice(start, end)   | 提取字符串的某个部分并在新字符串中返回被提取的部分，如：var res = str.slice(7,13); 如果参数为负，则从字符串的结尾开始计数，字符串最后一个字符表示-1；如果省略第二个参数，则该方法将裁剪字符串的剩余部分 |
| substring(start, end) | `substring()` 类似于 `slice()`，不同之处在于 `substring()` 无法接受负的索引 |
| substr(start, length) | `substr()` 类似于 `slice()`，不同之处在于第二个参数规定被提取部分的长度，如果省略第二个参数，则该 substr() 将裁剪字符串的剩余部分，如果首个参数为负，则从字符串的结尾开始计算位置 |
|       replace()       | 用另一个值替换在字符串中指定的值，var n = str.replace(str, str)，前一个str为原先字符串中的值，后一个str为需要替换进去的值，该方法不会改变调用它的字符串，它返回的是新字符串，该方法默认只是替换首个匹配到的字符串，不能进行全部替换，如需替换所有匹配，请使用正则表达式的 `g` 标志（用于全局搜索）如：var n = str.replace(/my/g, "our");注意包裹后不带引号；同时对大小写区分敏感，如需执行大小写不敏感的替换，请使用正则表达式 `/i`（大小写不敏感），如：var n = str.replace(/MY/i, "our");通过//i包裹原先的字符串，注意包裹后不带引号 |
|     toUpperCase()     |                  把字符串全部字符转换为大写                  |
|     toLowerCase()     |                  把字符串全部字符转换为小写                  |
|       concat()        | 连接两个或多个字符串，该方法可用于代替加运算符，text = text1.concat(" ",text2);方法第一个参数表示连接方式，例子表示用空格连接，将text2用空格连接到text1字符串后面 |
...
|        split()        | 将字符串转换为数组，其方法的参数表示用什么进行分割，如txt.split(","); 表示用逗号进行分隔，如果省略分隔符，被返回的数组将包含index [0] 中的整个字符串 |
|       repeat()        |        重复输出，效果类似于字符串相乘："*".repeat(3)         |

所有字符串方法都会返回新字符串，它们不会修改原始字符串，字符串是不可变的：字符串不能更改，只能替换

不建议对字符串进行属性访问（str[int]），因为：如果找不到字符，`[ ]` 返回 `undefined`，而 `charAt()` 返回空字符串；它是只读的。`str[0] = "A"` 不会产生错误（但也不会生效），如果想要进行属性访问，可以先把它转换为数组
...
console.log(phone(13456670599, 6))
//结果显示为：13456******
```
...
//结果显示为
{"jlc","24"}
{"我的名字是","，年龄为","。"}  //如果没有默认字符串，其前后也会加上空：{"","",""}，字符串的数量会大于变量的
```
...
let text = "9"
//隐式转换：
console.log(text * 1 + 9)  //结果显示18
//通过构造函数进行转换
console.log(Number(text) + 9)  //结果显示18
```
...
array.join("");
//通过构造函数进行转换
array.toString();  //默认是通过使用逗号进行连接的
```

#### 对象

为什么要进行面向对象的学习：1.使用函数编程会导致复杂的代码块，函数编程会暴露到全局，可能带来一系列问题，函数编程需要改良成面向对象的编程，将函数作为对象的一个方法，将关联性的业务封装起来，变成一个对象
...
JavaScript对象中的  名称:值对被称为属性，属性：属性值
...
默认的 case 不必是 switch 代码块中最后一个 case，如果 `default` 不是 switch 代码块中最后一个 case，请记得用 break 结束默认 case
...
`for in` 语句遍历对象的属性，或者遍历数组，主要用于从后台抓取.json数组数据进行批量处理
...
`for in`循环在老版本ES5中比较常用，在新版本中`for of`用的比较多
...
<script>
    ...
    `)
    //循环批量的处理数据
    for (let value of person) {  //对于数据，i就是每一个的编号
...
##### 循环中的break和continue关键字

`break` 语句“跳出”循环；`continue` 语句“跳过”循环中的这一个一个迭代，继续执行后续的循环
...
label1: for(let i = 1; i <= 10; i++){
    label2: for(let n = 1; n <= 10; n++){
        ...
        if(n + i > 10){
            break label1;  //调出label1标签的循环，如果不加标签，仅仅是退出当前的循环，及label2循环
        }
    }
}
```
