...
任何一个容器都可以指定为Flex布局，其display属性值可以设置为flex或inline-flex（行内元素布局）
...
Tailwind CSS 是一个备受欢迎的、基于原子类的CSS框架

原子类是一个类，对应于一个特定的 CSS 属性和值的组合。通过使用这些原子类，开发者可以直接在 HTML 中应用样式，而不必手动编写和管理一堆的 CSS 样式规则
...
在原先的全局样式文件中写入：原先的是`style.css`，可以修改为`index.css`，并在`main.js`文件中进行全局引入，`index.css`的内容为：
...
w-16 表示设置宽度为 4rem，h-8 表示设置高度为 2rem
