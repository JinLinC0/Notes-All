...
首先需要对 `pinia` 进行安装：`npm install pinia`

其次需要在`main.ts`文件中进行引入和挂载：
...
`action`中可以编写一些业务逻辑；`state`用于存放具体的数据

当`state`中的数据，需要经过处理后再使用时，可以使用`getters`配置

我们一般在`src/store/loveTalk.ts`路径下写一个.ts文件进行数据的存储，其文件的基本形式为：
